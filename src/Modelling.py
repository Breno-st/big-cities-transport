...
def _unique_rel_kpis(pairs):
    ...
    with open('/mnt/c/Users/b_tib/coding/Python/gMsc/big-cities-transport/weighted_time_by_segment_MTT_Total.csv', mode='r') as infile:
        for row in csv.DictReader(infile):
            o_d_tup = row["Segment"].replace("(", "").replace(")", "").replace(" ", "").split(',')
            o_d = o_d_tup[0]+'_'+o_d_tup[1]
            distress['MTT'][0][o_d] = float(row["Distress"])

    with open('/mnt/c/Users/b_tib/coding/Python/gMsc/big-cities-transport/weighted_time_by_segment_SUN_Total.csv', mode='r') as infile:
        for row in csv.DictReader(infile):
            o_d_tup = row["Segment"].replace("(", "").replace(")", "").replace(" ", "").split(',')
            o_d = o_d_tup[0]+'_'+o_d_tup[1]
...
if __name__ == '__main__':
    ...
    periods = [2, 3, 6, 3, 3, 3] #number of hours in each period
    days = ['MTT', 'SUN']
        ...
        if isinstance(list(UDR[kpi].values())[0], float): # check if first item is a float
            ...
            for day in days:
                ...
                if kpi == 'loads' or kpi == 'distress':
                    generate_edges(kpi_day, UDR[kpi][day][0]) # for each KPI an Db
