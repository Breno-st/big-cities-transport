...
def _unique_rel_kpis(mode, pairs):
    ...
    sun_path = '/mnt/c/Users/b_tib/coding/Python/gMsc/big-cities-transport/weighted_time_by_segment_SUN_Total.csv'
    ...
    output = {'distance':distance,
        'speed':speeds,
        'traffic':traffics,
        'distance-traffic':traffic_distances,
...
def _merge_lines(relations):
    ...
    for i in range(len(relations)):
        ...
        for t in range(6):
            # people by hour
            traffics['MTT'][t+1] += relations[i]['r']._properties['traffic_MTT'][t] / periods[t] # improve property traffic:{'day':[]}
            traffics['SUN'][t+1] += relations[i]['r']._properties['traffic_SUN'][t] / periods[t]
            # vehicles by hour
            frequencies['MTT'][t+1] +=  relations[i]['r']._properties['frequency_MTT'][t] / periods[t] # improve property frequency:{'day':[]}
            frequencies['SUN'][t+1] +=  relations[i]['r']._properties['frequency_SUN'][t] / periods[t]
    ...
    speed = sum(x * y for x, y in zip(line_speeds, line_traffic)) / sum(line_traffic)
...
def generate_edges(db, dict):
    ...
    with driver.session(database=db) as session:
        ...
        for id_pair in id_pairs:
            ...
            create = "MATCH ({0}:tubeStation),({1}:tubeStation) WHERE ID({0})={2} AND ID({1})={3} CREATE ({0})-[{4}:TO{{{5}:{6}}}]->({1})".format(start, end, ids[0], ids[1], seg, db, kpi_value)
...
if __name__ == '__main__':
    ...
    for mode in ods:
        ...
        for kpi in UDR.keys():
            ...
            if isinstance(list(UDR[kpi].values())[0], float): # check if first item is a float
                minmax_dict(UDR[kpi])

                create_db(mode, kpi)
                generate_nodes(db, mode) # day independent
                generate_edges(kpi,  UDR[kpi]) # db, kpi_name, dictionaire
            else:
                for day in days:
                    if kpi != 'distress':
                        for p in range(len(periods_hours)):
                            ...
                            create_db(mode, day, kpi) # day dependent
                            generate_nodes(db, mode) # inside create database
                            generate_edges(kpi,  UDR[kpi]) # db, kpi_name, dictionaire // inside create database


                    kpi_day = kpi+day.lower()
                    if kpi == 'loads' or kpi == 'distress':
                        generate_edges(kpi_day, UDR[kpi][day][0]) # for each KPI an Db // inside create database
...
## (12-Jun) Lundi:      5h: bike, 8h: Gym, 9h-18h: Work, 19h: diner, 20h: studying, 22h: reading
## (13-Jun) Mardi:      5h: bike, 8h: Gym, 9h-18h: Work, 19h: diner, 20h: studying, 22h: reading
## (14-Sep) Mecredi:    5h: bike, 8h: Gym, 9h-18h: Work, 19h: diner, 20h: studying, 22h: reading
## (15-Jun) Jeudi:      5h: bike, 8h: Gym, 9h-18h: Work, 19h: diner, 20h: studying, 22h: reading
## (16-Jun) Vendredi:   5h: bike, 8h: Gym, 9h-18h: Work, 19h: diner, 20h: studying, 22h: reading
## (10-Jun) Samedi:     5h: bike, 8h: Gym, 9h: , 13h: , 14h: Abnit, 18h: , 19h: diner, 20h: studying, 22h: reading
## (11-Jun) Dimache :   5h: gym, 6h: organizing, 9h: cleaning , 13h:Voluntee , 14h: , 18h: , 19h: , 20h: , 22h:

## BUY

## Bike: Chainring
## Office: Monitor, dock, keyboard
